<!DOCTYPE html>
<html lang="en">
<head>
 <link rel="stylesheet" href="css/style.css" type="text/css" media="screen">
<style>
input[type="text"], input[type="password"], input[type="submit"]
{
    border: none;
    background:silver;
    height: 50px;
    font-size: 16px;
	margin-left:35%;
	padding:15px;
	width:33%;	
	border-radius: 25px;
}
</style>
</head>
<body>
<div class="header">
  <a href="#default" class="logo"><img class="logo" src="img/Logo1.png"></a>
  <div class="header-right">
    <a href="index.jsp">Home</a>
    <a class="active" href="donorLogin.jsp">Donor Login</a>
  </div>
</div>
<body>
<div class="container">
<br>
	<br>
        <%
            String msg=request.getParameter("msg");
            if("invalid".equals(msg))
            {
                %>
                <center><font color="red" size="5">Invalid User name/Password</center>
                <%
            }
        %>
            
                <form action="donorLoginAction.jsp" method="post">
            <div class="form-group">
                <center><h2>Donor Id</h2></center>
                <input type="text"placeholder="Enter Donor Id" name="did"required 
                       oninvalid="this.setCustomValidity('Enter Donor Id Here')"
           oninput="this.setCustomValidity('')">
                <center><h2>Password</h2></center>
                <center><font color="red" size="3">*Use Mob_No As Password</center>
                <input type="password"placeholder="Enter Password"name="password"required
                       oninvalid="this.setCustomValidity('Enter Password Here')"
    oninput="this.setCustomValidity('')">
                <center><button type="submit"class="button">Login</button></center>        
        </form>
                <form action="dsignup.jsp" method="post">
            <div class="form-group">
                <center><button type="submit"class="button">Sign Up</button></center>        
        </form>
	
	
	
</div>
<br>
<br>
<br>
<br>
<h3><center>All Right Reserved @NITB ABHI :: 2021  </center></h3>
</body>
</html>
